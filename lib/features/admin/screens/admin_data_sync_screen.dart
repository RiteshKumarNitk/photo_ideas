...
class _AdminDataSyncScreenState extends State<AdminDataSyncScreen> {
  ...
  Future<void> _syncCategory(
    ...
    for (final entry in filters.entries) {
      ...
      if (subCategory == 'All') continue;

      for (final image in images) {
        try {
          await ApiService.uploadPhoto(
            imageUrl: image.url,
            posingInstructions: image.posingInstructions,
          );
          _addLog("Imported image for $category - $subCategory");
        } catch (e) {
          _addLog("Error importing image: $e");
        }
      }
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Sync Default Data')),
      body: Padding(
        padding: const EdgeInsets.all(16.0),
        child: Column(
          children: [
            const Text(
              "This tool will import all hardcoded 'Idea' images (Haircut, Wedding, etc.) into the database. This allows you to manage them in the Admin Dashboard.",
